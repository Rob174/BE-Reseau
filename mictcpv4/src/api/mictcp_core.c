...
/**
 * Keep string between boundaries with stop border excluded
 * @param str : input string to slice
 * @param start : index to start the slicing
 * @param stop : border to stop the slicing (slicing includes stop-1 not char of index stop)
 * */
char * slice(char*str,int start,int stop){
    ...
    char * buffer = malloc(sizeof(char)*(stop-start+1+1));
